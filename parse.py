...
def _is_horizontal_near(rect1: BaseGeometry, rect2: BaseGeometry, distance: float = 20) -> bool:
    """
...
def _merge_rects(rect_list: List[BaseGeometry], distance: float = 5, horizontal_distance: Optional[float] = None) -> \
        List[BaseGeometry]:
    """
...
def _parse_rects(page: fitz.Page) -> List[Tuple[float, float, float, float]]:
    ...
    # 忽略掉长度小于30的水平直线
    is_short_line = lambda x: abs(x['rect'][3] - x['rect'][1]) < 1 and abs(x['rect'][2] - x['rect'][0]) < 10
    ...
    merged_rects = _merge_rects(rect_list, distance=5, horizontal_distance=50)
    merged_rects = [rect for rect in merged_rects if explain_validity(rect) == 'Valid Geometry']

    # 过滤比较小的矩形
    merged_rects = [rect for rect in merged_rects if rect.bounds[2] - rect.bounds[0] > 60 and rect.bounds[3] - rect.bounds[1] > 60]
...
def _gpt_parse_images(
    ...
    def _process_page(index: int, image_info: Tuple[str, List[str]]) -> Tuple[int, str]:
        ...
        agent = Agent(role=role_prompt, api_key=api_key, base_url=base_url, model=model, disable_python_run=False)
...
def parse_pdf(
    ...
    image_infos,recs_info = _parse_pdf_to_images(pdf_path, output_dir=output_dir)
